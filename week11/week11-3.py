# week11-3.py LeetCode 2461. Maximum Sum of Distinct Subarrays With Length K
class Solution:
    def maximumSubarraySum(self, nums: List[int], k: int) -> int:
        ...
        for i in range(k,len(nums)): # 毛毛蟲慢慢從中間，走到最右邊
            ...
            total = total + nums[i] - nums[i-k] # 減左邊(舊)、加右邊(新)
            if counter[left]==0: del counter[left] # 減成0，就刪掉
            if len(counter)==k: ans = ans = max(ans,total)
        return ans